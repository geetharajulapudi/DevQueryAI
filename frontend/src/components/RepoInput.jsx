...
export default function RepoInput({ onIngest, loading, ingestStatus }) {
      ...
      <div className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          <div className={styles.inputRow}>
            ...
            <input className={styles.input} type="text" placeholder="https://github.com/owner/repository" value={url} onChange={(e) => setUrl(e.target.value)} disabled={loading} />
          </div>
          <button className={styles.btn} type="submit" disabled={loading || !url.trim()}>
            {loading ? <><span className={styles.spinner} /> {ingestStatus || "Processing..."}</> : "Analyze Repository →"}
          </button>
        </form>
